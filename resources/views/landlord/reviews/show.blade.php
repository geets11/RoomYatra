...
<div class="container mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        ...
        <div class="mt-4 md:mt-0">
            <a href="{{ route('landlord.reviews.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden mb-6">
                <div class="border-b border-gray-200 bg-gray-50 px-6 py-4">
                    <h2 class="text-lg font-medium text-gray-900">Guest Review</h2>
                </div>
                <div class="p-6">
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="border-b border-gray-200 bg-gray-50 px-6 py-4">
                    <h2 class="text-lg font-medium text-gray-900">Your Response</h2>
                </div>
                <div class="p-6">
                    @if($review->landlord_response)
                        <div class="mb-6">
                            <h3 class="text-sm font-medium text-gray-700 mb-2">Your current response:</h3>
                            <div class="p-4 bg-gray-50 rounded-md">
                                <p class="text-gray-800 whitespace-pre-wrap">{{ $review->landlord_response }}</p>
                            </div>
                            <p class="mt-2 text-xs text-gray-500">Responded on {{ $review->updated_at->format('M d, Y') }}</p>
                        </div>

                        <form action="{{ route('landlord.reviews.respond', $review->id) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="landlord_response" class="block text-sm font-medium text-gray-700 mb-2">Update your response:</label>
                                <textarea name="landlord_response" id="landlord_response" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500" placeholder="Update your response to this review...">{{ old('landlord_response', $review->landlord_response) }}</textarea>
                                <p class="mt-1 text-sm text-gray-500">Your response will be visible to all users who view this review.</p>
                            </div>
                    ...
                    @else
                        <form action="{{ route('landlord.reviews.respond', $review->id) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="landlord_response" class="block text-sm font-medium text-gray-700 mb-2">Write a response:</label>
                                <textarea name="landlord_response" id="landlord_response" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500" placeholder="Respond to this review...">{{ old('landlord_response') }}</textarea>
                                <p class="mt-1 text-sm text-gray-500">Your response will be visible to all users who view this review.</p>
                            </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white shadow-md rounded-lg overflow-hidden mb-6">
                ...
                <div class="p-6">
                    ...
                    <div class="mt-4">
                        <h4 class="text-sm font-medium text-gray-500">Property Rating</h4>
                        <div class="flex items-center mt-1">
                            <div class="flex items-center">
                                @for($i = 1; $i <= 5; $i++)
                                    @if($i <= $review->property->rating)
                                        <svg class="h-5 w-5 text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                        </svg>
                                    @elseif($i - 0.5 <= $review->property->rating)
                                        <svg class="h-5 w-5 text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                        </svg>
                                    @else
                            ...
                            <span class="ml-2 text-sm text-gray-700">{{ number_format($review->property->rating, 1) }} ({{ $review->property->reviews_count }} {{ Str::plural('review', $review->property->reviews_count) }})</span>
                        </div>
                    </div>

                    <div class="mt-6">
                        <a href="{{ route('landlord.properties.show', $review->property->id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500">
                            View Property
                        </a>
                    </div>
                </div>
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <div class="p-6">
                    <div class="mb-4">
                        <h4 class="text-sm font-medium text-gray-500">Guest</h4>
                        <p class="mt-1 text-gray-900">{{ $review->user->name }}</p>
                        <p class="text-sm text-gray-500">{{ $review->user->email }}</p>
                    </div>

                    <div class="mb-4">
                        <h4 class="text-sm font-medium text-gray-500">Stay Period</h4>
                    ...
                    <div class="mb-4">
                        <h4 class="text-sm font-medium text-gray-500">Booking Status</h4>
                        <p class="mt-1">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('landlord.bookings.show', $review->booking->id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500">
                            View Booking
                        </a>
                    </div>
                </div>
